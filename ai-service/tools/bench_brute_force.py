...
关于口径，两头都要说清楚：
  ...
  - 这里是**纯计算**耗时（一次 numpy 矩阵乘法），不含网络、不含向量化。
    真实检索还要先调一次向量化接口把问题转成向量，那一趟**实测 163 ~ 302 ms**
    （2026-09-25 用 text-embedding-v4 多次实测取的值域），比下面所有数字都大。
...
def main() -> int:
    ...
    print("     实测参考：一次向量化接口调用 163 ~ 302 ms（text-embedding-v4 实测），")
    print("     比上表最后一行还大一个数量级。")
